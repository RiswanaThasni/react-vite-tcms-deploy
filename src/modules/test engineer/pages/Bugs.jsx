...
import { useNavigate, useParams } from 'react-router-dom';
import { ArrowLeft, Upload, CheckCircle } from 'lucide-react';
import { getTestById, completeTestCases } from '../../../api/testApi';
...
const Bugs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Get the user_test_case_id from testData
      const userTestCaseId = testData.user_test_case_id;
      
      if (!userTestCaseId) {
        throw new Error("User test case ID not found. Please try again.");
      }
      
      // Prepare bug data according to backend requirements
      const bugData = {
        bug_id: bugId || undefined, // Only send if provided
        title: bugTitle,
        description: bugDescription,
        priority: priority,
        severity: severity.toLowerCase(), // Backend expects lowercase severity
        steps_to_reproduce: stepsToReproduce,
        environment: environment
      };
      
      // Use completeTestCases with properly formatted data
      await completeTestCases(
        userTestCaseId, 
        "completed", 
        bugData, 
        attachment
      ...
      // Set bugSubmitted to true to disable the button
    ...
    <div className="p-4">
      ...
      <div className="bg-white p-4 rounded shadow-sm border border-gray-200">
        ...
        <div className="mb-6 p-4 bg-blue-50 rounded-md border border-blue-200">
          ...
          {testData && testData.user_test_case_id && (
            <p className="text-sm text-blue-700 mt-1">
              User Test Case ID: {testData.user_test_case_id}
            </p>
          )}
        </div>
...
App Version: 2.4.1"
          ...
          <button
              ...
              } text-white px-4 py-2 rounded disabled:bg-gray-400 transition-colors`}
              disabled={submitting || !bugTitle || !bugDescription || bugSubmitted}
            >
              {bugSubmitted 
                ? 'Bug Reported' 
                : submitting 
