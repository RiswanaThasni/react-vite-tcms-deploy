  ...
  useEffect(() => {
    ...
    const isDueDate = (day) => {
      return dueDates.some((item) => {
        const dueDay = new Date(item.due_date).getDate();
        return dueDay === day;
      });
    };
    
    // Function to get upcoming deadlines - Fixed to match backend response structure
...
/>
    ...
    <>
      ...
      <div className="bg-card3 p-4 rounded shadow-sm">
        ...
        <p className="text-2xl font-bold text-white">{summary.pending_test_cases}</p>
          ...
          {/* Recently Opened Test Cases */}
          <div className="bg-white p-4 rounded shadow-sm border border-gray-200 mb-6">
            <h2 className="text-lg font-medium mb-4">Recent Test Cases</h2>
  ...
  tileClassName={({ date }) => {
    ...
    // Check if this date matches any due date
