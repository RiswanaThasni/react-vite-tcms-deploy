          ...
          <div className="overflow-x-auto bg-white rounded-xl shadow">
            {loading ? (
              ...
              <table className="w-full border-collapse rounded-lg">
                <thead className="bg-gray-100 border-b border-gray-300 font-light">
                ...
                <tbody>
                  {sortedProjects.map((project) => (
                    <tr key={project.project_id} className="text-gray-700 bg-white hover:bg-gray-100 transition border-b border-gray-300"
                    // onClick={()=>{ navigate(`/projectmanager_dashboard/project_details`)}}
                    >
                      <td className="p-3">{project.project_id}</td>
                      <td className="p-3">{project.project_name}</td>
                      <td className="p-3">{project.project_lead?.name || "N/A"}</td>
                      ...
                      <td className="p-3">
                        <div className="relative w-20 h-2 bg-gray-300 rounded">
                          <div
                            className={`absolute h-2 rounded ${getStatusColor(project.status)}`}
                            style={{ width: `${project.progress}%` }}
                          ></div>
                        </div>
                      </td>
                      <td className="p-3">
                        <span className={`px-2 py-1 text-white text-xs rounded-md ${getStatusColor(project.status)}`}>
                          {project.status}
                        </span>
                      </td>
                      <td className="p-3">{project.created_at.split("T")[0]}</td>
                      <td className="p-3">{project.deadline}</td>
                      <td className="p-3 text-center">{project.issues_count || "No Issues"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
