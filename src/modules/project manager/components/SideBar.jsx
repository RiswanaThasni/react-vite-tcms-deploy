    ...
    <>
      ...
      <div className="hidden md:flex flex-col items-center bg-gray-100 fixed left-0 top-0 h-full w-20 py-6">
        ...
        <div className="flex flex-col flex-grow items-center gap-4">
          <NavButton icon={home} alt="Home" path="/projectmanager_dashboard/mainsection" />
          <NavButton icon={overview} alt="Project Management" path="/projectmanager_dashboard/project_management" />
          <NavButton icon={taskmanagement} alt="Task Management" path="/projectmanager_dashboard/task_management" />
          <NavButton icon={reportanalysis} alt="Report Analysis" path="/projectmanager_dashboard/report_analysis" />
        </div>
      ...
      {/* Mobile Sidebar */}
      <div className={`md:hidden fixed top-0 left-0 h-full w-20 bg-white shadow-lg transition-transform duration-300 z-50 
        ...
        <button
          ...
          onMouseEnter={() => setHoverLogout(true)}
          onMouseLeave={() => setHoverLogout(false)}
          className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex items-center justify-center p-2 hover:bg-gray-200 relative"
        >
          <img src={logout} className="w-5 h-5 cursor-pointer hover:opacity-75" alt="Logout" />
          {hoverLogout && (
            <span className="absolute left-full ml-2 bg-black text-white text-xs px-2 py-1 rounded shadow-lg whitespace-nowrap">
